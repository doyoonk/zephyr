...
- 로컬 임포트용 bare: `/home/myrose/zephyrproject-rtos/zephyr.git`
- 이 문서와 `tools/zvendor` 는 원격의 `meta` orphan 브랜치에 있습니다 (아래 참조)
...
> 작업과 검증은 항상 clone 에서 하고, 원격에는 `main` / `b_vX.Y` 와
> **예외인 `meta` 브랜치**만 존재해야 합니다.
...
같은 해시가 나옵니다. 저장소를 재생성해도 결과가 동일합니다.

`refs/vendor/*` 는 브랜치가 아니라서 `git branch` 와 GitHub 브랜치 목록에는
`main`, `b_v4.3`, `b_v4.4` 만 보입니다. 일반 clone 에는 따라오지 않지만 필요하면:
...
`zvendor add` 가 중단되면서 해결용 명령을 출력합니다. **bare 에 worktree 를 만들지
