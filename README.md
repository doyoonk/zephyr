...
|/
* 00bb962e3 2026-04-14 (main)      zephyr v4.4.0
|
...
`main` 은 `b_v4.4` 체인 위가 아니라 **v4.4.0 지점**에 있습니다. `b_v4.3` 과 마찬가지로
`b_v4.4` 도 `main` 에서 갈라져 나온 유지보수 브랜치입니다.

실제로 `git log --graph --all` 을 돌리면 `main` 이 `b_v4.4` 와 같은 열에 그려집니다.
`main` 의 tip 이 정확히 v4.4.0 커밋이고 `b_v4.4` 가 그 자손이라 git 이 둘을 한 줄기로
묶어 표시하기 때문입니다. 분기 구조 자체는 위 그림이 맞고, `main` 에 내부 수정이
하나라도 올라가면 git 출력도 위와 같은 모양이 됩니다.

아직 내부 수정이 없어 모든 태그가 fast-forward 로 붙어 있습니다. 내부 커밋이 생긴
뒤부터 `Merge zephyr vX.Y.Z into b_vX.Y` 머지 커밋이 생깁니다.
